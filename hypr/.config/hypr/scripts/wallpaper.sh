...
#     "$SHUFFLE_ICON"
magick -size "${THUMBNAIL_WIDTH}x${THUMBNAIL_HEIGHT}" xc:#1e1e2e \
    \( "$HOME/.config/hypr/scripts/shuffle.avif" -resize "80x80" \) \
...
generate_menu() {
    ...
    for img in "$WALLPAPER_DIR"/*.{jpg,jpeg,png,webp,avif,svg}; do
        # Skip if no matches found
        [[ -f "$img" ]] || continue
        
        # Generate thumbnail filename
        thumbnail="$CACHE_DIR/$(basename "${img%.*}").avif"
...
if [ -n "$selected" ]; then
    ...
    if [[ "$thumbnail_path" == "$SHUFFLE_ICON" ]]; then
        ...
        original_path=$(find "$WALLPAPER_DIR" -type f \( -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.avif" -o -iname "*.webp" \) | shuf -n 1)
    else
    ...
    if [ -n "$original_path" ]; then
        ...
        #hyprctl hyprpaper wallpaper "eDP-1, $original_path, cover"
        hyprctl hyprpaper wallpaper "$monitor, $original_path, cover"
